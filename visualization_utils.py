...
from object_detection.core import standard_fields as fields # SH finish
from object_detection.utils import shape_utils # SH finish
...
####################### HM function #######################
def draw_summary_box_on_image(image,
                               class_count,
                               category_index,
                               classes,
                               thickness=4,
  ...
  if numbofboxes>0:
    ...
    listofdetectedclasses = []
    numbofdetectedclasses = [0] * numbofboxes

    for i in range(numbofboxes):
      if category_index[classes[i]]['name'] in listofdetectedclasses:
          for j in range(kinds):
              if listofdetectedclasses[j] == category_index[classes[i]]['name']:
                  numbofdetectedclasses[j]+=1
      else:
         listofdetectedclasses.append(category_index[classes[i]]['name'])
         numbofdetectedclasses[kinds]+=1
         kinds+=1

    #arrange in decent order
    quicksort(listofdetectedclasses, numbofdetectedclasses, 0, kinds-1)
    
    # list of result in array
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    ...
    #draw.rectangle(rectangleshape, fill='White', outline = 'black')
    ...
    header_s=' '.join(headers)
    rec = font.getmask(header_s).getbbox()
    
    table=[]
    for i in range(kinds):
      percentage = round(numbofdetectedclasses[i]/numbofboxes *100,2)
      temp = [listofdetectedclasses[i],
              str(percentage) + "%",
              str(numbofdetectedclasses[i]) + "/" + str(numbofboxes)]
      table.append(temp)

    # init table
    summary_box = tabulate(table, headers, tablefmt="outline", colalign=("left","right","right"))
    
    # draw summary box
    (x, y) = (im_width-(1.55*rec[2]), 1)
    draw.text((x, y), summary_box, fill=FontColor, font=font)
...
def visualize_boxes_and_labels_on_image_array(
  ...
  #draw summary box (HM)
  draw_summary_box_on_image(image,
                            class_count,
  	                        category_index,
  	                        classes,
  	                        line_thickness)

  csvdata = getcsvdata(image,
                       class_count,
                       category_index,
                       classes,
                       box_to_color_map,
...
# Quicksort Util 1 (HM)
def partition(array, array2, begin, end):
    pivot = begin
    for i in range(begin+1, end+1):
        if array[i] > array[begin]:
            pivot += 1
            array[i], array[pivot] = array[pivot], array[i]
            array2[i], array2[pivot] = array2[pivot], array2[i]
    array[pivot], array[begin] = array[begin], array[pivot]
    array2[pivot], array2[begin] = array2[begin], array2[pivot]
    return pivot

# Quicksort Util 2 (HM)
def quicksort(array, array2, begin=0, end=None):
    if end is None:
        end = len(array) - 1
    def _quicksort(array, array2, begin, end):
        if begin >= end:
            return
        pivot = partition(array, array2, begin, end)
        _quicksort(array, array2, begin, pivot-1)
        _quicksort(array, array2, pivot+1, end)
    return _quicksort(array, array2, begin, end)